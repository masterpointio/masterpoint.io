# Careers section — working guide

The `/careers/` section is where we publish job postings so we can send
...
- One canonical, on-brand URL per role (`/careers/<slug>/`) that we can link
...
```text
content/careers/
├── _index.md                                   # /careers/ banner copy + intro
└── YYYY-MM-DD-<slug>.md                        # one file per posting (blog-style date prefix)

layouts/careers/
├── list.html                                   # /careers/ — open roles, past openings, links
└── single.html                                 # job post page (hero, article card, apply)

layouts/partials/
├── careers-entry.html                           # job card used on /careers/
└── careers-notion-form.html                     # #apply section with the embedded Notion form

assets/css/careers.scss                          # all styling, @imported last in custom.scss
```

Other touch points:

- `config.yaml` → `menu.main`: top-level **Careers** item (weight 50, after the
  Content dropdown). The footer menu renders it automatically.
- `layouts/partials/menu.html` + `footer-menu.html`: the **Careers** nav item is
  highlighted on job post pages too (`eq $current.Section "careers"`), same trick
  the blog uses.
- `layouts/partials/head.html`: `JobPosting` JSON-LD emitted for pages in the
  `careers` section (`.IsPage` only, not the index).

## List page (`/careers/`)

- Body `id="careersPage"` (from `_index.md`). Same glow banner as the case
...
- **Open Roles**: every page in the section whose `status` is not `closed`,
  newest first, rendered by `careers-entry.html` as a media-left card with
...
- Deliberately **no** `schedule-assessment` sales CTA on careers pages; the
...
slug: senior-platform-engineer-fall-2026 # URL: /careers/<slug>/
...
applicant_countries: ["United States", "Canada"] # JobPosting.applicantLocationRequirements (default: United States)
...
preview_image: /img/bg_our_team.jpg # card image on /careers/
...
"!", so it needs a `<!-- markdownlint-disable MD026 -->` / `enable` pair around
it. About Masterpoint links to the values section (`/who-we-are/#who-our-values`)
with the `button` shortcode instead of repeating the values in the JD.
...
3. **Apply** (`#apply`, dark). Hidden when `status: closed`. Otherwise, in
   ...
   - `notion_form` set → `careers-notion-form.html`: a plain **Application**
...
- The iframe has a fixed height (1780px desktop, 2400px mobile in
  `careers.scss`) because Notion embeds do not auto-resize. If the form gains
...
1. Copy the most recent post in `content/careers/` to
   `content/careers/YYYY-MM-DD-<slug>.md`, update front matter and body.
...
of the article points at `/careers/`. Do not delete the file; keep the URL.
Optionally remove the role from the Notion form's **Position** options.

## Decisions & gotchas

- **Nav position.** `Careers` sits _after_ the Content dropdown on purpose.
...
- **`bg_our_team.jpg` is a placeholder preview/banner image.** Swap in a
  ...
  (`twitter-header.png`) would work if exported to `static/img/careers/`.
