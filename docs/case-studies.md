...
| `csi-split`       | Text + visual side-by-side; `flip="true"` alternates sides.    | `eyebrow`, `title`, `media`, `media_alt`, `figure` (CSS-drawn figure partial from `layouts/partials/case-studies/figures/<name>.html` instead of an image — e.g. `figure="terralith"`), `flip`, `contain`, `caption`, `variant`, `ratio` (`50-50` default / `65-35` / `75-25`, text wider — the ratios auto-reverse their column widths under `flip`, since flip moves the media into the first grid track via `order`) |
