...
- **Card click targets:** title, client logo, photo, and CTA all link to the study.
  Only the title and CTA are real links (focusable, announced); the logo and photo
  links are mouse/touch shortcuts (`tabindex="-1"` + `aria-hidden` +
  `data-csh-mirror`, which the slider's focus management skips) so keyboard / SR
  users don't hit the same destination four times. The blurb stays plain text
  (selectable). Hovering any card link (`.csh-card:has(a:hover)`) lifts the CTA and
  zooms the photo. **No title underline**: it slices through the `.text-gradient`
  spans and looks broken.
