...
date: 2025-03-06
description: 'In this follow-up to our ["What Is a Terralith"](https://masterpoint.io/updates/terralith-monolithic-terraform-architecture/) article, we shift the focus from describing the problem to providing a detailed migration plan, practical guidance, and a handy checklist for breaking up a Terralith into smaller, more manageable root modules.'
...
- [Why It’s Bad: Pitfalls \& The Scalability Ceiling of Terraliths](#why-its-bad-pitfalls--the-scalability-ceiling-of-terraliths)
        ...
        - [State File Bloat: Plans and Applies Slow Down](#state-file-bloat-plans-and-applies-slow-down)
...
Do you manage you’ve been managing your infrastructure with Terraform or OpenTofu (collectively referred to from here on out as TF)? Have you noticed long `plan` and `apply` durations? Or are you concerned about the blast radius of changes? If so, you might be dealing with a **Terralith**. “Terralith” refers to a monolithic TF architecture where all your resources are managed within a single root module and state file. This setup leads to inefficiencies and increased risk when making changes.
...
In this guide, we’ll walk you through the steps to break up a Terralith, helping you improve your infrastructure’s maintainability, scalability, and overall performance.
...
- **Large State File(s)**: A large number of resources are stuffed into a single `.tfstate` state file. There’s no universally correct file size or resource count for TF. However, our experience suggests that between 30 to 100 **resources per state file** is the range that strikes a good balance between optimal performance and manageability. Ideally, your TF root modules around service boundaries that share the same lifecycle, allowing for easy division.
...
- **High Blast Radius**: If even a small change or upgrade in one part of the infrastructure can affect unrelated components, congrats, you probably have a Terralith. This situation increases operational risk.
...
For example, within the same ECS environment, you might separate the deployment into:

1. A dedicated module for the ECS cluster and its core configuration.
1. One or more independent modules for individual ECS services.
...
3. **Create New Module Structures**
   1. For each new root module that you plan to introduce, create a separate directory.
   2. Move the relevant resource configurations from the Terralith into the new root modules.
   3. Initialize each new root module to ensure the backend is configured correctly.

4. **Inter-Module Communication (if any)**
   1. If any root module depends on outputs from another, update the references using data sources, variables, or remote state. We recommend using data sources over [`terraform_remote_state`](https://developer.hashicorp.com/terraform/language/state/remote-state-data) whenever possible.
...
5. **Move Resources to The New State**
   ...
   1. **Declarative Migrations Using TF 1.5+**
...
If you are not using TF 1.7+, you should remove the resources using the CLI. We’ll look at that option next.
   ...
   2. **Migration Using CLI Commands**
       ...
       **Importing and Removing resources**
       ...
       1. [*Importing*](https://opentofu.org/docs/cli/import/) existing resources to bring them into a new root module state, using `import`:

           ```hcl
           tofu import module.github_repos["foo"] "foo-repo"
           ```

       2. [*Removing*](https://opentofu.org/docs/cli/commands/state/rm/) resources from the Terralith root module’s state (you simply stop tracking certain resources without destroying them in the underlying infrastructure), using `state rm`:
       ...
       **Are there any caveats?**
       ...
       - Always backup your state files before performing any state manipulations.
...
![Terralith Breakup Checklist](/img/updates/steps-to-break-up-a-terralith/checklist.png)
