---
title: "MarketSpark · From ClickOps to 100% IaC"
slug: marketspark-case-study
description: "How Masterpoint rebuilt MarketSpark's AWS environment from a single ClickOps-managed account into an 11-account AWS Organization, 100% codified in OpenTofu, with 100+ AWS ECS and Lambda services across Development, Staging, and Production. Handed off to engineers who were already shipping against it."

# Hero
eyebrow: "CASE STUDY"
client: "MarketSpark"
client_logo: /img/case-studies/marketspark/client-logo.png
hero_title: "Modernizing &amp; migrating MarketSpark's manually-managed AWS Cloud to <span class='text-gradient'>100% Infrastructure as Code</span>."
hero_aside_image: /img/case-studies/marketspark/hero-bg.jpg
hero_aside_alt: "MarketSpark: connectivity in the field"
# Subtitle intentionally omitted; the stat strip below the hero carries the headline numbers.

# At-a-glance stat strip (appears under hero)
stat_bar:
  - value: "1 → 11"
    label: "AWS accounts · monolith to Organization"
  - value: "0% → ~100%"
    label: "Infrastructure as Code coverage"
  - value: "100+ Services"
    label: "AWS ECS & Lambda codified with <a href='https://en.wikipedia.org/wiki/Principle_of_least_privilege' target='_blank' rel='noopener'>PoLP</a> across Development, Staging, and Production environments"
...
# Closing quote from MarketSpark
client_quote:
  body: |
    <em>Placeholder quote, to be replaced with MarketSpark's words on the engagement.</em>
  attribution: "MarketSpark Engineering"
  role: "Platform Team"
...
{{< cs-callout title="About MarketSpark" icon="fa-building" variant="info" >}}
...
{{< cs-callout title="Why account isolation, not VPC isolation?" icon="fa-circle-question" variant="accent" >}}
...
{{< cs-figure src="/img/case-studies/marketspark/account-isolation.svg" alt="Before: one monolithic AWS account. After: eleven isolated accounts in an AWS Organization." caption="From a single shared account to eleven function- and environment-isolated accounts inside an AWS Organization." wide="true" >}}
...
{{< cs-callout title="Business impact" icon="fa-arrow-trend-up" variant="info" >}}
...
{{< cs-figure src="/img/case-studies/marketspark/iac-coverage.svg" alt="IaC coverage went from 0% to approximately 100%" caption="Every resource type is now declared in OpenTofu: networking, data, compute, messaging, edge, security." wide="true" >}}
...
{{< cs-todo >}}Show Spacelift Automation. Drop in a real screenshot of the Spacelift dashboard or a stack run.{{< /cs-todo >}}

{{< cs-pullquote attribution="The shape of the engagement" >}}
...
- **FinOps visibility, built in.** A uniform tagging strategy was applied across every resource in every account. Combined with account isolation, MarketSpark can allocate cost by environment and service for the first time. Cost runaways in dev no longer touch production billing.
    ...
    {{< cs-todo >}}Link Masterpoint's null context label blog post here.{{< /cs-todo >}}

The IoT networking re-architecture in particular is worth a closer look:

{{< cs-todo >}}Abstracted diagram of NAT on VPC Subnet Route Table level. Current placeholder below is illustrative; swap with the real architecture diagram.{{< /cs-todo >}}

{{< cs-figure src="/img/case-studies/marketspark/iot-networking.svg" alt="Before, VPN logic lived in application code on EC2; after, it lives at the network layer in VPC route tables and NAT." caption="The application team no longer owns VPN plumbing. Network changes are a route-table PR." wide="true" >}}
...
{{< cs-todo "placeholder quote" >}}Quote below is temporary / made up. Replace with a real MarketSpark engineer voice from the retro.{{< /cs-todo >}}

{{< cs-pullquote attribution="— MarketSpark engineer (placeholder)" >}}
