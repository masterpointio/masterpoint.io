...
MarketSpark had outgrown the AWS environment they started on, especially as the company's customer base grew, product surface widened, and they began to get compliance framework requests. Development, Staging, and Production all lived inside a single monolithic AWS account, and every change was a manual operation ([ClickOps](https://spacelift.io/blog/what-is-clickops)) in the AWS Console.
...
- **Centralized [Datadog](https://datadog.com) observability**, with APM, tracing, logs, and monitors, all defined in IaC and baked in.
...
- [Principle of Least Privilege (PoLP)](https://en.wikipedia.org/wiki/Principle_of_least_privilege) applied everywhere. With every IAM and security resource being rebuilt in IaC, Masterpoint engineers treated it as the perfect opportunity to codify least-privilege across all surfaces end-to-end.
...
body: Developer productivity increased as infrastructure ships the same way as application code. Infrastructure is democratized GitOps and automated with Spacelift instead of manually operating by hand in the AWS Console. Centralized observability means MarketSpark catches incidents in minutes instead of hearing about them from customers.
---
