...
The default _standard_ retry mode backs off **per request**: each throttled call politely waits, doubles, waits again, with no awareness of what any other in-flight request is doing. Terraform is concurrent with a default `-parallelism` of 10 resource operations at once, and each operation is rarely a single API call (there's the create or update, the read-back to refresh state, and often a polling waiter checking whether the resource is ready). While one request sleeps through its backoff, nine other requests are firing fresh calls, and expired backoffs re-collide with new traffic. In an enterprise setting, where multiple operations are running at the same time from multiple systems, this compounds!
